CREATE PROC [Finance].[ProcessActuals] ( @transitTable varchar(50) NULL,@Account varchar(50) NULL,@DateOfFile date NULL,@fileName varchar(1000) NULL) AS
	...
	BEGIN TRY
		...
		BEGIN-- @Account is not NULL, add Account to [Finance].[InvestmentAccount] if not already loaded
			...
			--then add instrument codes not already existing
				...
				ON [_Code_] =[kInstrumentCodeID] OR [_Code_] =[kAlternateCodeID]
				OR fi.[k2ndAlternateID] LIKE  (SELECT [_Code_] + '%')
				OR [_Code_] LIKE  (SELECT [k2ndAlternateID]  + '%')
			...
			--*/
				...
				ON [_Code_] =[kInstrumentCodeID] OR [_Code_] =[kAlternateCodeID]
				OR [k2ndAlternateID] LIKE  (SELECT [_Code_] + '%')
				OR [_Code_] LIKE  (SELECT [k2ndAlternateID]  + '%')
				) as trans
...
-- Clear out  transactions
	DELETE FROM [Finance].[InvestmentTransactionsTransit]
		...
		END
		IF @transitTable = '[Finance].[Inv Transactions a]'

		BEGIN
			UPDATE [Finance].[Inv Transactions a]
			...
			from  [Finance].[Inv Transactions a]
  ...
  --,len
			...
			FROM [Finance].[Inv Transactions a] rg (nolock)
			LEFT OUTER JOIN [Finance].[FinTran] (nolock) ft
			...
			DELETE FROM [Finance].[Inv Transactions a]

		END
		
		IF @transitTable = '[Finance].[Inv Transactions c]'

		BEGIN

			UPDATE [Finance].[Inv Transactions c]
			SET 
			[Debits] = STUFF(STUFF([Debits],PATINDEX('%"%',[Debits]),1,''),PATINDEX('%"%',STUFF([Debits],PATINDEX('%"%',[Debits]),1,'')),1,''),
			[Credits] = STUFF(STUFF([Credits],PATINDEX('%"%',[Credits]),1,''),PATINDEX('%"%',STUFF([Credits],PATINDEX('%"%',[Credits]),1,'')),1,'')
			from  [Finance].[Inv Transactions c]
			...
			UPDATE [Finance].[FinTran]
			SET  [Card Number] = 
			rc.[Card Number] 
			--SELECT rc.*
			FROM  [Finance].[Inv Transactions c] rc (NOLOCK)
			LEFT OUTER JOIN [Finance].[FinTran] rg(NOLOCK)
			ON rg.[Posted Date]= rc.[Posting Date]
			AND rg.[Transaction Date]= rc.[Transaction Date]
			AND rg.[Description]=rc.[Description]

			DELETE FROM [Finance].[Inv Transactions c]
		END
